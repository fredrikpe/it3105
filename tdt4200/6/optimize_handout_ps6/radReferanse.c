...
void addValue(AccurateImage *imageOut, AccurateImage *b, int a, int j) {
  imageOut->data[a].red += b->data[j].red;
  imageOut->data[a].blue += b->data[j].blue;
  imageOut->data[a].green += b->data[j].green;
}
void subtractValue(AccurateImage *imageOut, AccurateImage *b, int a, int j) {
  imageOut->data[a].red -= b->data[j].red;
  imageOut->data[a].blue -= b->data[j].blue;
  imageOut->data[a].green -= b->data[j].green;
}
void performNewIdeaIteration(AccurateImage *imageOut, AccurateImage *b, AccurateImage *imageIn, int size) {
  ...
  for( int y=0; y<H; y++)
  {
    for(int x=0; x<W; x++)
    {
      b->data[y*W + x].red = imageIn->data[y*W + x].red;
      b->data[y*W + x].blue = imageIn->data[y*W + x].blue;
      b->data[y*W + x].green = imageIn->data[y*W + x].green;
      if (x)
      {
        if (y)
        {
  ...
  for( int y=0; y<H; y++)
  {
    ...
    /*
      if (y-size >= 0)
      {
        if (x-size >= 0) // p1
        {
          imageOut->data[p1].red += b->data[y*W + x].red;
          imageOut->data[p1].blue += b->data[y*W + x].blue;
          imageOut->data[p1].green += b->data[y*W + x].green;
        }
        if (x+size+1 < W) // p2
        {
          imageOut->data[p2].red -= b->data[y*W + x].red;
          imageOut->data[p2].blue -= b->data[y*W + x].blue;
          imageOut->data[p2].green -= b->data[y*W + x].green;
        }
      }
      if (y+size+1 < H)
      {
        if (x-size >= 0) // p3
        {
          imageOut->data[p3].red -= b->data[y*W + x].red;
          imageOut->data[p3].blue -= b->data[y*W + x].blue;
          imageOut->data[p3].green -= b->data[y*W + x].green;
        }
        if (x+size+1 < W) // p4
        {
          imageOut->data[p4].red += b->data[y*W + x].red;
          imageOut->data[p4].blue += b->data[y*W + x].blue;
          imageOut->data[p4].green += b->data[y*W + x].green;
        }
      }
      if (x > (W-size-1))
      {
        if (y > (H-size-1)) // extra p2, 3, 4
        {
          p2 = (H-1)*W + (x - 1);
          p3 = (y-1)*W + W - 1;
          p4 = H*W - 1;
...
/*double getNewValue(double old_value) {
  if (old_value < -1)
    old_value += 257;
  if(old_value > 255)
    return 255;
...
PPMImage * performNewIdeaFinalization(AccurateImage *is, AccurateImage *il, PPMImage *imageOut) {
  ...
  for(int i = 0; i < W * H; i++) {
    double value = (il->data[i].red - is->data[i].red);
