...
AccurateImage *convertImageToNewFormat(PPMImage *image) {
	...
	for(int i = 0; i < image->x * image->y; i++) {
		imageAccurate->data[i].red   = (storage_t) image->data[i].red;
		imageAccurate->data[i].green = (storage_t) image->data[i].green;
		imageAccurate->data[i].blue  = (storage_t) image->data[i].blue;
	}
...
void performNewIdeaIteration(AccurateImage *imageOut, AccurateImage *imageIn, int colourType, int size) {
	...
	for(int senterX = 0; senterX < imageIn->x; senterX++) {
		...
		for(int senterY = 0; senterY < imageIn->y; senterY++) {
			...
			storage_t valueR = (storage_t) (sumR / countIncluded);
			storage_t valueG = (storage_t) (sumG / countIncluded);
			storage_t valueB = (storage_t) (sumB / countIncluded);
...
int main(int argc, char *argv[]) {
	...
	PPMImage *final_tiny = performNewIdeaFinalization(imageAccurate2_tiny,  imageAccurate2_small);//convertNewFormatToPPM(imageAccurate2_small);
