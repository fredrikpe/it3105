...
void performNewIdeaIteration(AccurateImagePlanes *imageOut, AccurateImagePlanes *imageIn, int colourType, int size) {
	...
	// Compute the sum in the top left box
	compute_t sumBorder = 0;
	for(int y = -size; y <= size; y++) {
		for(int x = -size; x <= size; x++) {
			int currentX = size + x;
			int currentY = size + y;
			...
			int numberOfValuesInEachRow = imageIn->x; 
			int offsetOfThePixel = (numberOfValuesInEachRow * currentY + currentX);
			sumBorder += imageIn->colors[colourType][offsetOfThePixel];
		}
	}
	
	// Now we can add the right border and subtract the left border
	
	
	// Iterate over each pixel in the center region
	for(int centerY = size+1; centerY < (imageIn->y - size); centerY++) {
	
		// First we remove the top values
		for(int x = -size; x <= size; x++) {
			int currentX = size + x;
		...
		// Now we compute the final value
		storage_t valueBorder = (storage_t) (sumBorder * rep);
		// Update the output image
		int numberOfValuesInEachRow = imageOut->x;
		int offsetOfThePixel = (numberOfValuesInEachRow * centerY + size);
		imageOut->colors[colourType][offsetOfThePixel] = valueBorder;
		
		
		
		// Start with the border value
		compute_t sum = sumBorder;
		
		// Now we move one pixel to the right, and compute the delta
		for(int centerX = size+1; centerX < (imageIn->x - size); centerX++) {
			// For each pixel we compute the delta
			
			//printf("sum start: %f, add: %f\n", sum);
			
			// First we remove the left values
			for(int y = -size; y <= size; y++) {
